...
一个正常模块是不应该用数字命名的，还有一般情况下一个模块不应该有直接执行的功能代码，因为这种代码在导入的时候被执行一般是不被期待的，所以我们用`if __name__ == "__main__"`语句来判断：
...
这种方式的一种变通就是`from package import *`，但此时等于导入`__init__.py`中定义的所有的内容，但仅限于这个文件中定义的内容
